...
$app->get('/m_rules/trash', function ($request, $response) {
  $params = $request->getParams();
  $db = Db::db();
  $data = $db->select('*')->from('m_rules')->where('is_deleted', '=', 1);
...
$app->post('/m_rules/restore', function ($request, $response) {
  $params = $request->getParams();
  $db = Db::db();
  $model = $db->update('m_rules', ['is_deleted' => 0], ['id' => $params['id']]);
...
$app->post('/m_rules/delete_permanen', function ($request, $response) {
  $params  = $request->getParams();
  $db    = Db::db();
  $model = $db->delete('m_rules', ['id' => $params['id']]);
