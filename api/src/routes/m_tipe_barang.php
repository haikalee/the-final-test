...
$app->get('/m_tipe_barang/trash', function ($request, $response) {
  $params = $request->getParams();
  $db = Db::db();
  $data = $db->select('*')->from('m_tipe_barang')->where('is_deleted', '=', 1);
...
$app->post('/m_tipe_barang/restore', function ($request, $response) {
  $params = $request->getParams();
  $db = Db::db();
  $model = $db->update('m_tipe_barang', ['is_deleted' => 0], ['id' => $params['id']]);
...
$app->post('/m_tipe_barang/delete_permanen', function ($request, $response) {
  $params  = $request->getParams();
  $db    = Db::db();
  $model = $db->delete('m_tipe_barang', ['id' => $params['id']]);
